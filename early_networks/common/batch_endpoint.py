"""
batch_endpoint.py - batch our examples with angles and endpoints
...
def init_data_sets(FLAGS, gen_data) :
  ...
  if not os.path.exists(FLAGS.pickle_filename):
    ...
    training_input, training_output, training_end = angles_to_array(training_angles, FLAGS)
    validate_input, validate_output, validate_end = angles_to_array(validate_angles, FLAGS)
    test_input, test_output, test_end = angles_to_array(test_angles, FLAGS)

    # I wanna see you pickle it (just a little bit)
    pickle_it(FLAGS.pickle_filename,(training_input, training_output, validate_input, validate_output, test_input, test_output, training_end, validate_end, test_end))

  else:
    print("Loading from pickle file")
    with open(FLAGS.pickle_filename, 'rb') as f:
      training_input, training_output, validate_input, validate_output, test_input, test_output, training_end, validate_end, test_end = pickle.load(f)

  return (training_input, training_output, validate_input, validate_output, test_input, test_output, training_end, validate_end, test_end)
...
def next_batch(input_data, output_data, output_end, FLAGS) : 
  ...
  ix = np.zeros((v,FLAGS.max_cdr_length, FLAGS.num_acids))
...
def random_batch(input_data, output_data, output_end, FLAGS) : 
  ...
  ex = np.zeros((FLAGS.batch_size, 3))

  for i in range(0,FLAGS.batch_size):
    rnd = randint(0,len(input_data)-1) 
    ix[i] = input_data[rnd]
    ox[i] = output_data[rnd]
    ex[i] = output_end[rnd]
...
def angles_to_array(angle_set, FLAGS):
  ...
  print(len(fu), len(fu[0]), len(ft), len(ft[1]), len(ed), len(ed[1]))
  input_set = np.array(ft,dtype=np.bool)
  output_set = np.array(fu,dtype=np.float32)
  end_set = np.array(fe, dtype=np.float32) 

  return (input_set, output_set, end_set)
