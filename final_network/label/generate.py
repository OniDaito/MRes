    ...
    with open(FLAGS.save_path + "/model_lookup.txt","w") as f:
      ...
      while bt.has_next_batch(settype) and count < (end - start):
        ...
        for j in range(0, FLAGS.batch_size):
          ...
          if (len(str(loops[j])) <= 7 and abdb_only) or (not abdb_only):

            with open(FLAGS.save_path + "/" + str(count+start+j).zfill(4) + "_real.txt","w") as rw: 
              with open(FLAGS.save_path + "/" + str(count+start+j).zfill(4) + "_pred.txt","w") as pw: 
                for i in range(0,len(batch_in[j])):  
                  ...
                  rw.write(acids.amino_to_label(res_name) + "," + str(math.degrees(phi)) + "," + str(math.degrees(psi)) + "\n")  

                  if FLAGS.type_out == batcher.BatchTypeOut.SINCOS:
                    phi = math.atan2(res[0][j][i][4], res[0][j][i][1])
                  ...
                  pw.write(acids.amino_to_label(res_name) + "," + str(math.degrees(phi)) + "," + str(math.degrees(psi)) + "\n")
            real_models.append(real_residues)
            test_models.append(test_residues)
          
            loop_models.append(loops[j])
            f.write( str(idx+start).zfill(4) + "," + str(loops[j]) + "\n")
            idx += 1
            count += 1
